...
import styles from './Edior.module.css'
...
function Editor(props) {
    ...
    const workExpBody = (
        <div className={styles.detail}>
            <div className={styles.row}>
                ...
                <InputControl
                    label="Company Name"
                    placeholder="Enter name such as amazon"
    ...
    const projectBody = (<div className={styles.detail}>
        <div className={styles.row}>
            <InputControl
                label="Title"
                placeholder="Enter title eq. Chat App"
                value={values.title}
                onChange={(event) => setValues((prev) => ({ ...prev, title: event.target.value }))}
            />
        </div>
    ...
    const achievementBody = (
        <div className={styles.detail}>
            <div className={styles.column}>
                <label>List all your achievements</label>
                <InputControl
                    placeholder="Line 1"
                    value={values.points ? values.points[0] : ""}
                    onChange={(event) => handlePointUpdate(event.target.value, 0)}
    ...
    const skillsBody = (
        <div className={styles.detail}>
            <InputControl
                ...
                value = {values.skills || ''}
                onChange = {(event) => setValues((prev)=> ({...prev, skills: event.target.value}))}
            />
        </div>
    )
    const summaryBody = (
    ...
    const extraCurricular = (<div className={styles.detail}>
        <div className={styles.column}>
            <label>List all your all your co-curricular/ extra-curricular achivements</label>
            <InputControl
                placeholder="Line 1"
                value={values.points ? values.points[0] : ""}
                onChange={(event) => handlePointUpdate(event.target.value, 0)}
    ...
    const basicInfoBody = (<div className={styles.detail}>
        <div className={styles.row}>
            <InputControl
                label="Name"
...
case sections.skills: {
    ...
    // Ensure that tempPoints is an array. If not, wrap it in an array.
    const pointsArray = Array.isArray(tempPoints) ? tempPoints : [tempPoints];

    // Wrap each entry in an object with the key 'point'
    const wrappedPoints = pointsArray.map(point => ({ point }));

    props.setInformation((prev) => ({
        ...prev,
        [sections.skills]: {
            ...prev[sections.skills],
            details: [...prev[sections.skills].details, ...wrappedPoints],
        },
    }));
    break;
}
    ...
    const handleDeleteDetail = function (index) {
        const details = activeInformation?.details
    ...
    useEffect(() => { setActiveInformation(information[sections[activeSectionKey]]); }, [information])
    ...
    useEffect(() => {
        ...
        setValues({
            ...
            name: activeInfo.details[activeDetailIndex]?.college || "",
        });
    }, [activeDetailIndex]);
        ...
        <div className={styles.container}>
            ...
            <div className={styles.body}>
                <InputControl label="Title"
                    placeholder="Enter Section Title" v
                    value={sectionTitle}
                ...
                <div className={styles.chips}>
                    {
                        activeInformation?.details ?
                            activeInformation?.details?.map((item, index) => (
                                ...
                                >
                                    <p> 
                                    {sections[activeSectionKey]} {index + 1}</p> 
                                    <X
                                        onClick={(event) => {
